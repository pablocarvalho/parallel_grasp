...
Dmer::Dmer(const char * name, int MaxIter, int MaxTime, int target)
{ 
  /* tempo inicial */
  t_ini  = (unsigned long int) clock();

  strcpy(nome, name);
  le_dados_grasp(MaxIter, MaxTime, target);   /* carrega a estrutura custo a partir do arquivo de entrada e executa grasp */
...
void Dmer::le_dados_grasp(int MaxIter, int MaxTime, int target)
{ 
  //int           jj;
  char          arq[256];
  std::ifstream fin; 
  ...
  for (int i = 0;i < qtdExecucoes; i++) {
    ...
    int jj = sg.grasp_sig_v2(sg.vet1, f, sg.vet2, f2, sg.vet3, f3, sg.vet4, f4, f5, f6, MaxIter, MaxTime, TEST, target);  
    cout<< "\nExecucao: " << i+1 << "---------------------------------------------------------------------------------"<<endl;
